...
parsed_df = parsed_df.withColumn('transaction_dt_ts',
                                 unix_timestamp(parsed_df.transaction_dt, 'dd-MM-YYYY HH:mm:ss').cast(TimestampType()))
...
def lTransD_data(a):
    hdao = dao.HBaseDao.get_instance()
    data_fetch = hdao.get_data(key=a, table='look_up_table')
    return data_fetch['info:transaction_dt']
...
parsed_df = parsed_df.withColumn('last_transaction_date_ts',
                                 unix_timestamp(parsed_df.last_transaction_date, 'YYYY-MM-dd HH:mm:ss').cast(
                                     TimestampType()))
...
parsed_df = parsed_df.withColumn('last_transaction_date_ts',
                                 unix_timestamp(parsed_df.last_transaction_date, 'YYYY-MM-dd HH:mm:ss').cast(
                                     TimestampType()))
parsed_df = parsed_df.withColumn('time_taken',
                                 time_udf(parsed_df.last_transaction_date_ts, parsed_df.transaction_dt_ts))
...
def status_find(card_id, member_id, amount, pos_id, postcode, transaction_dt, transaction_dt_ts,
                last_transaction_date_ts, score, speed):
    hdao = dao.HBaseDao.get_instance()
    ...
    row = {'info:postcode': bytes(postcode), 'info:pos_id': bytes(pos_id), 'info:card_id': bytes(card_id),
           'info:amount': bytes(amount), 'info:transaction_dt': bytes(transaction_dt),
           'info:member_id': bytes(member_id), 'info:status': bytes(status)}
    key = '{0}.{1}.{2}.{3}'.format(card_id, member_id, str(transaction_dt), str(datetime.now())).replace(" ",
                                                                                                         "").replace(
        ":", "")
...
parsed_df = parsed_df.withColumn('status',
                                 status_udf(parsed_df.card_id, parsed_df.member_id, parsed_df.amount, parsed_df.pos_id,
                                            parsed_df.postcode, parsed_df.transaction_dt, parsed_df.transaction_dt_ts,
                                            parsed_df.last_transaction_date_ts, parsed_df.score, parsed_df.speed))
